...
const hash = require("../lib/hashtables");
const fs = require("fs");
 ...
 */
function similarity_score(input, movie_arr) {
    if (movie_arr == undefined) {
        return 0;
    }
    ...
    if (relevant_movies_watched === 0)
        return 0;
    return (total_rating / relevant_movies_watched) * Math.pow(relevant_movies_watched, 0.5);
}
...
/**
  ...
  */
function getRelevantUsers(movies, file_path, min_number, user_movie_table) {
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(file_path)
            ...
            const movie = Number(row.movieId);
 ...
 */
function main(input_movies, file_path, min_number) {
    return __awaiter(this, void 0, void 0, function* () {
        // init hashtable for relevant users
        const user_movie_table = hash.ph_empty(330975, hash.hash_id);
        /// init hashtable for movie and score
        const movie_score_table = hash.ph_empty(288983, hash.hash_id);
        // init hashtable for keeping trrack of users similarity score
        const sim_table = hash.ph_empty(330975, hash.hash_id);
        // init hashtable for keeping track of how many times a movie was rated to handle not recomending always popular mopvies
        const movie_count = hash.ph_empty(288983, hash.hash_id);
        yield getRelevantUsers(input_movies, file_path, min_number, user_movie_table);
        const keys = hash.ph_keys(user_movie_table);
        // computes and adds similarity scores for each user to simTable
        list.for_each((key) => {
            const m_array = hash.ph_lookup(user_movie_table, key);
            const sim_score = similarity_score(input_movies, m_array);
            if (sim_score > 0)
                hash.ph_insert(sim_table, key, sim_score);
        }, keys);
        ...
        list.for_each((key) => {
            ...
            for (let i = 0; i < movie_arr.length; i = i + 1) {
                ...
                if (input_movies.includes(current_movie))
                    continue;
                ...
                hash.ph_insert(movie_score_table, current_movie, (current_score !== null && current_score !== void 0 ? current_score : 0) + vote);
                const current_count = hash.ph_lookup(movie_count, current_movie);
                hash.ph_insert(movie_count, current_movie, (current_count !== null && current_count !== void 0 ? current_count : 0) + 1);
            }
        }, keys);
